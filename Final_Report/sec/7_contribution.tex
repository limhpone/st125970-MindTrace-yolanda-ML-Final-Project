...
\begin{table}[!htb]
\centering
\caption[Part~A results]{Part~A results --- SVM test-set performance when each preprocessing step is removed. $\Delta$\,Acc and $\Delta$\,F1 are relative to the full-pipeline baseline.\protect\footnotemark}
...
\midrule
0 & Full pipeline       & 91.80 & 91.75 & 0.00   & 0.00   \\
1 & $-$ Lowercase       & 91.80 & 91.75 & 0.00   & 0.00   \\
2 & $-$ Whitespace      & 91.80 & 91.75 & 0.00   & 0.00   \\
3 & $-$ URL removal     & 91.78 & 91.73 & $-$0.02 & $-$0.02 \\
4 & $-$ Emoji removal   & 91.80 & 91.75 & 0.00   & 0.00   \\
5 & $-$ Special chars   & 91.80 & 91.75 & 0.00   & 0.00   \\
6 & $-$ Chat-word       & 91.77 & 91.72 & $-$0.03 & $-$0.03 \\
7 & $-$ Stopwords       & 90.47 & 90.40 & $-$1.33 & $-$1.35 \\
8 & $-$ Lemmatisation   & 91.70 & 91.66 & $-$0.10 & $-$0.09 \\
9 & Raw text            & 90.38 & 90.32 & $-$1.42 & $-$1.43 \\
\bottomrule
\end{tabular}
\end{table}
\footnotetext{Absolute values are anchored to the main-experiment SVM baseline (91.80\%, Table~\ref{tab:results}) for consistency. The ablation notebook's independent retrain yielded a marginally different baseline (91.97\%); all $\Delta$ values are computed from that run and are therefore exact.}

Removing \textbf{stopword removal} caused the largest single-step accuracy drop ($-1.33$\,pp), confirming that retaining semantically meaningful function words while removing noise tokens is critical for TF-IDF performance. \textbf{Lemmatisation} was the second most impactful step ($-0.10$\,pp), consolidating morphological variants into shared stems. Five steps (lowercasing, whitespace stripping, emoji removal, special character removal) had zero measurable impact on this corpus, likely because the TF-IDF vocabulary already absorbed their effects. Using raw text without preprocessing reduced accuracy by $1.42$\,pp, underscoring the cumulative value of the full pipeline. Figure~\ref{fig:ablation_svm} visualises the per-step impact; an alternative Accuracy-vs-F1 view is in Appendix Figure~\ref{fig:ablation_svm_f1}.
...
The BiLSTM baseline (2$\times$BiLSTM 256$\rightarrow$128, embedding~dim\,=\,100, dropout\,=\,0.5) was retrained with seven architectural variants spanning depth, capacity, regularisation, and embedding dimensionality. Table~\ref{tab:ablation_configs_b} lists the configurations; Table~\ref{tab:ablation_results_b} reports test-set performance.
...
\textbf{\#} & \textbf{Config} & \textbf{Acc\,(\%)} & \textbf{F1\,(\%)} & \textbf{$\Delta$\,Acc} & \textbf{$\Delta$\,F1} \\
\midrule
0 & Baseline            & 93.93 & 93.91 & 0.00   & 0.00   \\
1 & 1-layer (256)       & 92.71 & 92.71 & $-$1.22 & $-$1.20 \\
2 & 128$\rightarrow$64  & 93.61 & 93.61 & $-$0.32 & $-$0.30 \\
3 & 512$\rightarrow$256 & 94.22 & 94.23 & $+$0.29 & $+$0.32 \\
4 & Dropout 0.3         & 93.58 & 93.59 & $-$0.35 & $-$0.32 \\
5 & Dropout 0.7         & 93.44 & 93.41 & $-$0.49 & $-$0.50 \\
6 & Emb dim 50          & 93.39 & 93.41 & $-$0.54 & $-$0.50 \\
7 & Emb dim 200         & 94.02 & 94.03 & $+$0.09 & $+$0.12 \\
\bottomrule
\end{tabular}
\end{table}
