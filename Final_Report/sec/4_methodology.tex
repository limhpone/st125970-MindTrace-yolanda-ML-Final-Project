...
Table~\ref{tab:eda_summary} summarises per-class text length statistics. Posts are concentrated between 20--150 characters (right-skewed), with Love producing the longest posts (median 94 chars) and Sadness the shortest (median 81). These statistics informed maximum sequence length and padding strategies for DL models. A Pearson correlation analysis confirmed strong redundancy between \texttt{text\_length} and \texttt{word\_count} ($r = 0.984$), while correlation between label encoding and length features was near-zero ($r < 0.03$), confirming that classification depends on semantic content rather than message length. Full distribution plots are in Appendix~\ref{sec:appendix} (Figures~\ref{fig:text_length} and~\ref{fig:correlation_matrix}).
...
\midrule
Joy       & 2 & 465 & 98.8 & 88 \\
Sadness   & 2 & 493 & 93.1 & 81 \\
Anger     & 2 & 478 & 96.0 & 84 \\
Fear      & 3 & 487 & 96.7 & 85 \\
Love      & 3 & 471 & 104.7 & 94 \\
Surprise  & 3 & 459 & 99.7 & 89 \\
\midrule
...
\textbf{Data Splitting.} The dataset was split 80\% training / 20\% testing with a 20\% validation holdout from training data, using stratified sampling to preserve class proportions. After downsampling (89,832 total samples), this yielded 57,492 training, 14,373 validation, and 17,967 test samples. Although this single stratified split ensured controlled comparison across all model families, the resulting estimates may remain split-dependent; future work should incorporate $k$-fold cross-validation for more robust performance estimation.
