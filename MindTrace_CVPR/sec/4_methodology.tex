...
Figure~\ref{fig:text_length} presents the text length histogram and Figure~\ref{fig:wordcount} shows the word count boxplot by emotion class. The overall median text length is 86 characters (mean 97.0, std 56.2), with Love having the longest posts (median 94) and Anger the shortest (median 84). The histogram confirms a right-skewed distribution with posts concentrated between 20 and 150 characters. The boxplot reveals that Love has the highest median text length (median $\approx 94$ chars), while Anger is the most concise (median $\approx 69$ chars). These findings inform the maximum sequence length (512) and padding strategies for neural network architectures.
...
A Pearson correlation analysis revealed a strong positive correlation ($r = 0.984$) between \texttt{text\_length} and \texttt{word\_count}, confirming structural redundancy. The correlation between the numeric label encoding and length-based features is near-zero ($r < 0.03$), confirming that emotional classification depends on semantic content rather than message length, reinforcing TF-IDF and embedding-based representations as the essential feature extraction strategy.
...
\textbf{Preprocessing.} An eight-step NLP pipeline normalises raw text: (1)~lowercasing, (2)~whitespace stripping, (3)~URL removal, (4)~emoji removal, (5)~special character removal (retaining only \texttt{[a-z\textbackslash s]}), (6)~chat-word expansion (30 common abbreviations), (7)~stopword removal, and (8)~lemmatisation. Stopword removal uses the NLTK English stopword list; however, negation words and intensifiers (\eg, ``not'', ``never'', ``very'', ``extremely'') are retained via a custom exception list, as these tokens carry direct emotional polarity and their removal would cause loss of discriminative signal. Class imbalance is addressed through random downsampling to the minority-class size ($\approx$14,872 per class, 89,232 total).

\textbf{Feature Representation.} For machine learning models, TF-IDF vectorisation (\texttt{max\_features=5000}, \texttt{ngram\_range=(1,2)}) transforms text into numerical feature representations capturing both unigrams and bigrams:
\begin{equation}
...
\begin{tikzpicture}[
    ...
    mainbox/.style={rectangle, draw=gray!60, rounded corners=2pt, align=center,
                font=\sffamily\tiny, minimum width=2.4cm,
                minimum height=0.45cm, fill=#1, text width=2.4cm,
                line width=0.4pt},
